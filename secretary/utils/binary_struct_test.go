...
func TestBinaryStructSerialize(t *testing.T) {
	...
	for _, test := range tests {
		...
		binaryData, _ := SerializeBinaryStruct(test.s)
		t.Logf("\n\nSerializ: %+v\n", binaryData)

		var d testStruct
		DeserializeBinaryStruct(binaryData, &d)
		t.Logf("\n\nDeserial: %+v\n", d)

		hashS, _ := md5Struct(test.s)
		hashD, _ := md5Struct(d)

		eq, err := CompareBinaryStruct(test.s, d)
...
func TestCompareBinaryStruct(t *testing.T) {
	...
	for _, test := range tests {
		...
		equal, err := CompareBinaryStruct(test.a, test.b)
		if err != nil {
			t.Fatal(err)
		}
